...
import { format, addMinutes, startOfDay, endOfDay, isWithinInterval, parseISO } from "date-fns";
...
export async function getHairdressersWithServices(): Promise<HairdresserWithServices[]> {
  ...
  for (const hairdresser of hairdressers) {
    const serviceRelations = await db
      ...
      .leftJoin(servicesTable, eq(hairdressersServices.serviceId, servicesTable.id));
...
export async function getAvailableTimeSlots(
  ...
  const hairdresserServiceIds = serviceId 
    ? (await db
        ...
        .from(hairdressersServices)
        .where(eq(hairdressersServices.serviceId, serviceId))).map(h => h.hairdresserId)
    : [];
  ...
  const service = serviceId 
    ? await db.select().from(servicesTable).where(eq(servicesTable.id, serviceId)).limit(1)
    : null;
  ...
  bookingServices.forEach(service => {
    serviceDurations.set(service.id, parseInt(service.time_required.toString(), 10));
  });
  ...
  availabilities.forEach(avail => {
    ...
    while (addMinutes(currentSlotStart, serviceDuration) <= availEndTime) {
      ...
      const isAvailable = !existingBookings.some(booking => {
        ...
        return booking.hairdresserId === avail.hairdresserId && (
          (currentSlotStart < bookingEnd && slotEnd > bookingTime)
        );
      });
...
export async function createBooking(data: NewBooking) {
  ...
  const bookingEndTime = addMinutes(data.appointmentDate, serviceDuration);
  ...
  const actualConflicts = await Promise.all(
    conflictingBookings.map(async (booking) => {
      ...
      const bookingDuration = parseInt(bookingService[0].time_required.toString(), 10);
      const bookingTime = new Date(booking.appointmentDate);
      ...
      return (
        (newBookingTime < bookingEndTime && newBookingEndTime > bookingTime)
      );
    })
  );
